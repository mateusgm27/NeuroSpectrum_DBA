...
-- 2. Tabela: PROFISSIONAL (Psicólogos, Psiquiatras e Pgiscopedagogas)
...
CREATE TABLE consulta (
    ...
    data_hora DATETIME,
    duracao_min INT,
    status ENUM('Agendada', 'Cancelada', 'Realizada', 'Concluido', 'Em Andamento'),
...
-- 7. Tabela: PRONTUÁRIO (Evolução clínica e sessões)
...
-- 8. Tabela: RECEITA (Prescrições médicas emitidas)
...
-- 9. Tabela: DIAGNÓSTICO (Mapeamento de CIDs)
...
-- 11. Tabela: EXAME (Solicitações e laudos clínicos)
CREATE TABLE exame (
    ...
    status ENUM('Solicitado', 'Em Analise', 'Concluido', 'Cancelado') DEFAULT 'Solicitado',
    FOREIGN KEY (consulta_id) REFERENCES consulta (consulta_id) ,
    FOREIGN KEY (paciente_id) REFERENCES paciente (paciente_id) 
);

-- =============================================================================
-- 12. LÓGICA PROCEDURAL / OTIMIZAÇÃO (PGSQL)
-- =============================================================================
...
BEGIN
    ...
    -- O próprio MySQL faz a validação otimizada de conflito na agenda
    ...
    -- IF / ELSE exigido pelo professor para controle de regras de negócio
...
-- =============================================================================
-- 13. INSERÇÃO DE DADOS DE TESTE (DML)
-- =============================================================================
...
('Dra. Glória Pires', '606.606.606-60', 'CRP-DF 44556', 'Psicólogo', 'Suporte Emocional e Desenvolvimento', '(61) 99111-0006', 'gloria.psi@neuro.com', '2026-01-12 15:00:00'),
...
-- Prof_Disponibilidade Relacionamentos
...
-- Consultas Iniciais (Mapeadas conforme status reais do Figma)
