...
-- SELECT 1: Listagem Geral de Consultas (Mapeando Paciente, Profissional e Admin)
...
-- SELECT 2: Prontuários com Detalhes Clínicos e Evolução do Paciente
SELECT pron.prontuario_id AS id, p.nome AS paciente, pro.profissional_id AS profissional,
pron.data_registro AS data_registro, pron.queixa_principal AS queixa_principal, pron.avaliacao AS avaliacao
...
SELECT pro.nome AS profisional , pro.especialidade AS especialidade
...
-- SELECT 5: Relatório de Diagnósticos (CIDs) por Paciente
SELECT diag.descricao AS descricao, p.nome AS paciente, con.consulta_id AS consulta, prof.nome AS profissional
...
-- SELECT 6: Filtro de Pacientes por Convênio (Útil para faturamento da recepção)
SELECT nome FROM paciente WHERE convenio != 'particular';

-- SELECT 7: Próximas Consultas Agendadas (Foco em gerenciamento futuro)
...
-- SELECT 11: Busca por Nome de Paciente Específico (Exemplo: Filtrando por 'Enzo')
SELECT paciente_id AS id, nome AS nome, DATE_FORMAT(data_nascimento, '%d/%m/%Y') AS 'data_nascimento', telefone AS telefone, endereco AS endereco
FROM paciente
WHERE nome LIKE '%Enzo%'
ORDER BY nome;

-- SELECT 12: Total de Minutos de Atendimento Clínico por Profissional (Uso de SUM e GROUP BY)
...
-- SELECT 13: Média de Duração das Consultas por Modalidade (Uso de AVG)
SELECT AVG(duracao_min) FROM consulta GROUP BY modalidade;

-- SELECT 14: Ranking de Profissionais com Mais Pacientes em Prontuário
SELECT prof.tipo AS especialidade , prof.nome AS nome, COUNT(DISTINCT p.paciente_id) AS total_pacientes_unicos
FROM prontuario p
...
-- SELECT 15: Exames Clínicos que já possuem Resultados Concluídos
SELECT p.nome AS Paciente, e.tipo_exame AS Exame, e.resultado AS 'Resultado do Laudo', e.data_resultado AS 'Data de Conclusão'
FROM exame e
JOIN paciente p ON e.paciente_id = p.paciente_id
...
-- =============================================================================
-- TESTES DE VALIDAÇÃO DA PROCEDURE (PGSQL / PROCEDURAL)
-- =============================================================================
